import React from 'react';
import { NodeResizer, Handle, Position, useReactFlow } from 'reactflow';
import type { NodeProps } from 'reactflow';
import { Pump3DSwitch } from '../nodes/Pump3DSwitch';
import { Trash2 } from 'lucide-react';

function AdminNodeDeleteBtn({ id, nodeName, allowDelete, onDelete }: { id: string; nodeName?: string; allowDelete?: boolean; onDelete?: (id: string) => void }) {
  const [confirming, setConfirming] = React.useState(false);
  if (!allowDelete || !onDelete) return null;

  if (confirming) {
    return (
      <div
        onClick={(e) => e.stopPropagation()}
        style={{
          position: 'absolute', top: -16, right: -10, zIndex: 50,
          background: '#17181c', border: '1.5px solid #ef4444',
          borderRadius: 8, padding: '4px 8px',
          display: 'flex', alignItems: 'center', gap: 6,
          boxShadow: '0 6px 16px rgba(0,0,0,0.4)',
          animation: 'fadeIn 0.15s ease',
          fontFamily: "'Plus Jakarta Sans', 'Inter', system-ui, sans-serif"
        }}
      >
        <span style={{ fontSize: 11, fontWeight: 700, color: '#ffffff', whiteSpace: 'nowrap' }}>
          Delete {nodeName || 'Power Switch'}?
        </span>
        <button
          onClick={(e) => {
            e.stopPropagation();
            onDelete(id);
          }}
          style={{
            background: '#ef4444', color: '#fff', border: 'none', borderRadius: 4,
            padding: '3px 8px', fontSize: 10, fontWeight: 700, cursor: 'pointer', transition: 'opacity 0.15s'
          }}
        >
          Yes
        </button>
        <button
          onClick={(e) => {
            e.stopPropagation();
            setConfirming(false);
          }}
          style={{
            background: 'rgba(255,255,255,0.18)', color: '#fff', border: 'none', borderRadius: 4,
            padding: '3px 8px', fontSize: 10, fontWeight: 700, cursor: 'pointer', transition: 'opacity 0.15s'
          }}
        >
          No
        </button>
      </div>
    );
  }

  return (
    <button
      onClick={(e) => {
        e.stopPropagation();
        setConfirming(true);
      }}
      title="Delete Switch"
      style={{
        position: 'absolute', top: -10, right: -10, zIndex: 30,
        width: 26, height: 26, borderRadius: '50%',
        background: '#ef4444', color: '#ffffff', border: '2px solid #ffffff',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        cursor: 'pointer', boxShadow: '0 2px 8px rgba(239,68,68,0.4)',
      }}
    >
      <Trash2 size={13} strokeWidth={2.5} />
    </button>
  );
}

export function SmartGardenSwitchNodeView({ id, data, selected }: NodeProps<any>) {
  const nodeName = data?.nodeName || '3D Power Switch';
  const isFlipped = !!data?.flipHorizontal;
  const reactFlow = useReactFlow();

  const [switchScale, setSwitchScale] = React.useState(data?.switchScale ?? 0.5);

  React.useEffect(() => {
    if (data?.switchScale !== undefined) setSwitchScale(data.switchScale);
  }, [data?.switchScale]);

  const isOn = data?.pumpOn ?? data?.valveOn ?? data?.switchOn ?? true;

  const handleToggle = () => {
    const nextState = !isOn;
    reactFlow.setNodes((nds) =>
      nds.map((n) => {
        if (n.id === id) {
          return {
            ...n,
            data: {
              ...n.data,
              pumpOn: nextState,
              valveOn: nextState,
              switchOn: nextState,
            },
          };
        }
        return n;
      })
    );
  };

  return (
    <div
      style={{
        width: '100%',
        height: '100%',
        minWidth: 80,
        minHeight: 110,
        position: 'relative',
        background: 'transparent',
        display: 'flex',
        flexDirection: 'column',
        alignItems: 'center',
        justifyContent: 'center',
      }}
    >
      <AdminNodeDeleteBtn id={id} nodeName={nodeName} allowDelete={data?.allowDeleteNodes} onDelete={data?.onDeleteNode} />

      {data?.allowMoveResize && (
        <NodeResizer
          keepAspectRatio={true}
          minWidth={70}
          minHeight={90}
          isVisible={selected}
          onResizeStart={(_evt, params) => data?.onResizeStart && data.onResizeStart(params, id)}
          onResizeEnd={(_evt, params) => data?.onResizeEnd && data.onResizeEnd(params, id)}
          lineStyle={{ borderColor: '#00ffff', borderWidth: 2 }}
          handleStyle={{ background: '#00ffff', borderColor: '#17181c', width: 10, height: 10, borderRadius: 3 }}
        />
      )}

      {/* Target/Source Connection Handles */}
      <Handle
        type="target"
        position={Position.Left}
        id="inlet-1"
        style={{
          left: 0,
          top: '50%',
          width: 8,
          height: 8,
          background: '#10b981',
          border: '1.5px solid #ffffff',
          opacity: 0,
          zIndex: 50,
        }}
      />
      <Handle
        type="source"
        position={Position.Right}
        id="outlet-1"
        style={{
          right: 0,
          top: '50%',
          width: 8,
          height: 8,
          background: '#10b981',
          border: '1.5px solid #ffffff',
          opacity: 0,
          zIndex: 50,
        }}
      />

      {/* Interactive 3D Rocker Switch */}
      <div
        className="nodrag nopan"
        style={{
          transform: isFlipped ? 'scaleX(-1)' : 'none',
          transition: 'transform 0.25s ease',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
        }}
      >
        <Pump3DSwitch
          isOn={isOn}
          canControl={true}
          onToggle={handleToggle}
          scale={switchScale}
        />
      </div>
    </div>
  );
}
