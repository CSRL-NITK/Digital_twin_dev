...
async function main() {
  ...
  // Check if nodes already exist for this topology
  const existingNodesCount = await prisma.node.count({ where: { topologyId: topology.id } });
  if (existingNodesCount > 0) {
    console.log(`Topology already has ${existingNodesCount} nodes. Skipping node and edge creation to preserve saved positions.`);
  } else {
    // Create Nodes
    const nodesData = [
