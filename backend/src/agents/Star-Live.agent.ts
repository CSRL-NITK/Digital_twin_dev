...
export class StarLiveAgent {
  ...
  async handleQuery(userQuery: string): Promise<string> {
    ...
    const systemPrompt = `You are a human SCADA Operator and field engineer specialized in the Star Topology Water Distribution System.
...
2. Pay close attention to the "Status" column in the [NODES & SENSORS TELEMETRY] table to identify which specific nodes are currently "Critical", "Warning", or "Healthy".
3. IMPORTANT: If any node status is 'Critical' or 'Offline', report it immediately as a critical issue (e.g. Tank-2 is Critical, Pumps are Offline), even if the active alerts list is empty.
4. If asked about history or predictions, state casually: "Historical analysis and predictions are out of scope for this live view."
5. If a sensor value is not present, say: "Data not available for that sensor."
6. Never invent or hallucinate values. Keep answers to 2-3 sentences maximum.`;
  ...
  private async buildTelemetryContext(): Promise<string> {
    ...
    // Fetch active unread alerts for Star Topology nodes
...
Topology ID: ${this.topologyId} (Star Topology)
