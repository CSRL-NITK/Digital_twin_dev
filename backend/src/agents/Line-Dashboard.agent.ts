...
export class DashboardAgent {
  ...
  private topologyId: number = 5; // Default topology is Line Topology (ID 5)

  constructor() {
    // Dashboard uses the larger, more capable qwen2.5:3b model for better math analysis
...
4. Never invent or hallucinate data metrics. Keep answers to 2-3 sentences maximum.`;
  ...
  private async buildDashboardContext(): Promise<string> {
    ...
    const systemHealthScore = totalNodes > 0 ? (healthyNodesCount / totalNodes) * 100 : 100.0;
