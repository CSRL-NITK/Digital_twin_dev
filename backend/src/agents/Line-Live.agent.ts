...
export class LineLiveAgent {
  private ollama: OllamaProvider;
  private topologyId: number = 5; // Fixed for Line Topology
  ...
  async handleQuery(userQuery: string): Promise<string> {
    ...
    const systemPrompt = `You are a human SCADA Operator and field engineer specialized in the Line Topology Water Distribution System.
...
4. Never invent or hallucinate values. Keep answers to 2-3 sentences maximum.`;
  ...
  private async buildTelemetryContext(): Promise<string> {
    ...
    // Fetch active unread alerts for Line Topology nodes
...
Topology ID: ${this.topologyId} (Line Topology)
