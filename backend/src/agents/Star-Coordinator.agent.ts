...
import { StarLiveAgent } from './Star-Live.agent';
import { StarDashboardAgent } from './Star-Dashboard.agent';
import { StarSimulationAgent } from './Star-Simulation.agent';
import { SandboxState } from './Line-Simulation.agent';
import { prisma } from '../lib/prisma';

export class StarCoordinatorAgent {
  private ollama: OllamaProvider;
  private liveAgent: StarLiveAgent;
  private dashboardAgent: StarDashboardAgent;
  private simulationAgent: StarSimulationAgent;
  private topologyId: number = 1; // Fixed for Star Topology

  constructor() {
    // The Star Topology Coordinator uses the qwen2.5:3b model as its dispatcher brain
    this.ollama = new OllamaProvider('qwen2.5:3b');
    this.liveAgent = new StarLiveAgent();
    this.dashboardAgent = new StarDashboardAgent();
    this.simulationAgent = new StarSimulationAgent();
  }
  ...
  async handleQuery(userQuery: string, sandboxState?: SandboxState): Promise<string> {
    ...
    const classifierSystemPrompt = `You are the Star Topology Routing Assistant. 
Your only job is to classify which sub-agent must handle the user's query.
Reply with exactly one of these tags:
- [LIVE] if the query is about a specific single node/tank (e.g. Tank-1, Tank-2, Central Tank-734), single sensor readings (current pH, current level, etc.), or active warnings.
...
IMPORTANT: Questions asking about a specific tank name (like "Tank-1" or "Tank-2") must ALWAYS go to [LIVE]. Only global metrics or averages go to [DASHBOARD].
...
Your response MUST contain ONLY the tag, e.g., "[LIVE]" or "[DASHBOARD]" or "[SIMULATION]". Do not output any other text or reasoning.`;
    ...
    try {
      ...
      console.log(`[Star Coordinator] Routing query "${userQuery}" -> Target: ${route}`);
      ...
      console.error('[Star Coordinator Error]:', error.message);
  ...
  private async buildDefaultSandboxState(): Promise<SandboxState> {
    ...
    return {
      ...
      temperature: temp,
      isPumpRunning,
      logs: ['System initialized Star coordinator baseline state.']
    };
  }
}
